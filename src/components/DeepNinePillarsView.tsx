...
const PILLAR_ICONS: Record<PillarCategory, React.ReactNode> = {
  ...
  batters: <Activity className="w-3.5 h-3.5 text-blue-400" />,
  bullpen: <Shield className="w-3.5 h-3.5 text-emerald-400" />,
  injuries: <HeartCrack className="w-3.5 h-3.5 text-rose-400" />,
  lineup: <ListOrdered className="w-3.5 h-3.5 text-purple-400" />,
  statcast: <Gauge className="w-3.5 h-3.5 text-cyan-400" />,
  matchup: <Crosshair className="w-3.5 h-3.5 text-yellow-400" />,
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[#0e1017] border border-white/[0.08] rounded-2xl p-5 text-white shadow-sm">
            ...
            <button
              ...
              className={`px-3.5 py-2 rounded-xl font-bold text-xs flex items-center gap-2 transition-all ${
                isSimulatorMode
                  ...
                  : 'bg-zinc-900 hover:bg-zinc-800 text-cyan-300 border border-white/10'
              }`}
            >
        ...
        <div className="mt-4 pt-3 border-t border-white/[0.06] overflow-x-auto no-scrollbar">
          <div className="flex items-center gap-1.5 min-w-[760px] text-[10px] font-mono text-zinc-300">
            ...
            <span className="px-2 py-0.5 rounded-lg bg-blue-500/10 text-blue-300 border border-blue-500/20 flex items-center gap-1">
              <Activity className="w-3 h-3 text-blue-400" /> Bateo (18%)
            </span>
            <span className="text-zinc-600">+</span>
            ...
            <span className="px-2 py-0.5 rounded-lg bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 flex items-center gap-1">
              <Gauge className="w-3 h-3 text-cyan-400" /> Statcast (14%)
            </span>
            <span className="text-zinc-600">+</span>
            <span className="px-2 py-0.5 rounded-lg bg-yellow-500/10 text-yellow-300 border border-yellow-500/20 flex items-center gap-1">
              <Crosshair className="w-3 h-3 text-yellow-400" /> Matchup (9%)
            </span>
            <span className="text-zinc-600">+</span>
            ...
            <span className="text-zinc-600">+</span>
            <span className="px-2 py-0.5 rounded-lg bg-purple-500/10 text-purple-300 border border-purple-500/20 flex items-center gap-1">
              <ListOrdered className="w-3 h-3 text-purple-400" /> Lineup (4%)
            </span>
            <span className="text-zinc-600">+</span>
            ...
            <ArrowRight className="w-3.5 h-3.5 text-cyan-400 mx-1 shrink-0" />
            <span className="px-2.5 py-0.5 rounded-lg bg-zinc-800 border border-white/10 text-cyan-300 font-black shrink-0">
              KAL MODEL
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
          ...
          <div className="lg:col-span-8 bg-[#0e1017] border border-white/[0.08] rounded-2xl p-4 shadow-sm space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              {/* 1. Pitcher */}
              <div className="p-3 bg-zinc-950 rounded-xl border border-white/[0.06] space-y-1.5">
              ...
              <div className="p-3 bg-zinc-950 rounded-xl border border-white/[0.06] space-y-1.5">
                <div className="flex items-center justify-between text-xs font-mono">
                  <span className="font-bold text-yellow-400 flex items-center gap-1.5 text-[11px]">
          ...
          <div className="lg:col-span-4 bg-[#0e1017] border border-white/[0.08] rounded-2xl p-4 shadow-sm space-y-3 font-mono">
            <div className="pb-2.5 border-b border-white/[0.06]">
              <span className="text-[9px] font-bold text-cyan-400 uppercase tracking-wider">Inferencia Inmediata</span>
              <h3 className="text-sm font-black text-white">Probabilidad Polymarket</h3>
            </div>

            {/* Probability Dial */}
            <div className="p-4 bg-zinc-950 rounded-xl border border-white/[0.06] text-center space-y-2.5">
              <div className="text-[10px] text-zinc-500 uppercase">Ganador Simulado</div>
              ...
              <div className="flex items-center justify-between text-[10px] text-zinc-400 pt-1">
                <span>Confianza: <strong className="text-cyan-300">{simConfidence}</strong></span>
                <span>Ventaja: <strong className="text-white">{Math.abs(simHomeProb - 50).toFixed(1)}%</strong></span>
              </div>
            </div>

            {/* Inferred explanation */}
            <div className="p-3 bg-zinc-950 rounded-xl border border-white/[0.06] text-xs text-zinc-300 space-y-1.5 font-sans">
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 shrink-0 ${
                    isSelected
                      ? 'bg-zinc-800 text-white border border-white/20 shadow-xs'
                      : 'bg-[#0e1017] border border-white/[0.06] text-zinc-400 hover:text-white'
                  }`}
                >
                  <span>{g.matchup}</span>
                  <span className="font-mono text-[9px] px-1.5 py-0.2 rounded bg-black/40 text-cyan-400 font-bold">
                    {g.winner} {g.prob}%
                  </span>
                </button>
              );
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between font-mono">
                    <div className="flex items-center gap-2">
                      <div className="p-1 rounded-lg bg-zinc-950 border border-white/10">
                        {icon}
                      </div>
                      <div>
                        <h4 className="text-xs font-bold text-white">{pillar.name}</h4>
                        <span className="text-[9px] text-zinc-500">Peso: {pillar.weight_pct}%</span>
                      </div>
                    </div>
                  ...
                  {/* Metrics Box */}
                  <div className="p-2 bg-zinc-950 rounded-xl border border-white/[0.06] text-[10px] font-mono space-y-0.5">
                    <div className="text-zinc-300 truncate">
                      <strong className="text-zinc-500">Casa:</strong> {pillar.home_metric_display}
                    </div>
                    <div className="text-zinc-300 truncate">
                      <strong className="text-zinc-500">Visita:</strong> {pillar.away_metric_display}
                    </div>
                    {pillar.statcast_badge && (
                      <div className="pt-0.5 text-[9px] text-cyan-400 font-bold">
