...
function parseExplanationMetrics(exp: string): Record<string, any> {
  const text = String(exp || '');
  const out: Record<string, any> = {};

  // Example generated by the Python predictor:
  // "ERA temporada previa: Away 3.20 | Home 4.10"
  const era = text.match(/ERA temporada previa:\s*[^|\n]+?\s+([0-9]+(?:\.[0-9]+)?)\s*\|\s*[^|\n]+?\s+([0-9]+(?:\.[0-9]+)?)/i);
  if (era) {
    out.away_starter_era = Number(era[1]);
    out.home_starter_era = Number(era[2]);
  }

  // "OPS lineup: HOME .760 vs AWAY .710"
  const ops = text.match(/OPS lineup:\s*[^\s]+\s+([0-9]+(?:\.[0-9]+)?)\s+vs\s+[^\s]+\s+([0-9]+(?:\.[0-9]+)?)/i);
  if (ops) {
    out.home_ops_30d = Number(ops[1]);
    out.away_ops_30d = Number(ops[2]);
  }

  // "Parque: factor 1.083"
  const park = text.match(/Parque:\s*factor\s+([0-9]+(?:\.[0-9]+)?)/i);
  if (park) out.park_factor = Number(park[1]);

  const lineupStatus = text.match(/Lineup\s+[^\[]+\[([^\]]+)\]/gi);
  if (lineupStatus?.length) {
    const statuses = lineupStatus.map((x) => {
      const m = x.match(/\[([^\]]+)\]/);
      return m?.[1] || '';
    });
    if (statuses[0]) out.away_lineup_status = statuses[0].includes('confirm') ? 'confirmed' : 'projected';
    if (statuses[1]) out.home_lineup_status = statuses[1].includes('confirm') ? 'confirmed' : 'projected';
  }
...
export function mapApiPrediction(row: any): any {
  ...
  const parsed = parseExplanationMetrics(row.exp ?? row.explanation ?? '');
  return {
    ...parsed,
    ...row,
    game_pk: row.game_pk,
    ...
    home_starter_era: row.home_starter_era ?? parsed.home_starter_era,
    away_starter_era: row.away_starter_era ?? parsed.away_starter_era,
    home_starter_fip: row.home_starter_fip ?? parsed.home_starter_fip,
    away_starter_fip: row.away_starter_fip ?? parsed.away_starter_fip,
    home_starter_k9: row.home_starter_k9 ?? parsed.home_starter_k9,
    away_starter_k9: row.away_starter_k9 ?? parsed.away_starter_k9,
    home_ops_30d: row.home_ops_30d ?? parsed.home_ops_30d,
    away_ops_30d: row.away_ops_30d ?? parsed.away_ops_30d,
...
export async function fetchLiveStatus(): Promise<{ live: boolean; ok: boolean; raw?: any; error?: string }> {
...
export async function fetchLivePreds(date: string): Promise<any[] | null> {
  ...
  const load = async (d: string) => {
    const r = await fetch(`${API_BASE}/api/preds?date=${encodeURIComponent(d)}`, { cache: 'no-store' });
    if (!r.ok) return null;
...
export async function triggerCycle(secret: string): Promise<any> {
  ...
  const r = await fetch(`${API_BASE}/api/run/cycle`, { method: 'POST', headers: { 'x-kal-secret': secret } });
...
export function normalizeHistoryItem(r: any): HistoryItem & { home: string; away: string; pick: string; prob: number; conf: string; isGraded: boolean; isHit: boolean | null; units: number } {
  ...
  const hasValidFinalScores = rawHs !== null && rawAs !== null && !isNaN(rawHs) && !isNaN(rawAs) && !(rawHs === 0 && rawAs === 0) && rawHs !== rawAs;
  const g = r.graded;
  ...
  if (typeof game_date === 'number') game_date = new Date(game_date > 1e12 ? game_date : game_date * 1000).toISOString().slice(0, 10);
  return { ...r, game_pk: r.game_pk, game_date: game_date || '', home, away, pick, prob: Math.max(home_p, away_p), conf: conf === 'HIGH' || conf === 'MEDIUM' ? conf : 'LOW', isGraded, isHit, units, home_score: rawHs !== null && !isNaN(rawHs) ? rawHs : undefined, away_score: rawAs !== null && !isNaN(rawAs) ? rawAs : undefined, venue_name: r.venue_name };
}
